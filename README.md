...
- **database**:
  ...
  - All the game questions are stored in and retrieved from `localstorage` but will be replaced with database data later.
    - Lyric sentences to display as questions, four possible answer choices to select from, correct answer of the song name, and corresponding SoundCloud soundtrack link of the correct song are stored in and retrieved from `localstorage` for each game question.
...
- **Application Interaction Logic**
  ...
  - The game displays Lyric sentences to display as questions, four possible answer choices to select from, and some emoji reactions to click with (The emoji reaction clicked will be displayed with the other real-time notification messages on the left side of the "Play" page.).
...
# Login Deliverable
*Note: The Login Deliverable is completed over the week of March 31st, 2024 and the week of April 7th, 2024. Please look for and refer to the git commits made for this deliverable during these two weeks.
...
*Note: The WebSocket Deliverable is completed over the week of April 7th, 2024. Please look for and refer to the git commits made for this deliverable during that week.
- **Backend Listening for WebSocket Connection**: The `index.js` file calls the `peerProxy.js` file and `peerProxy()` class to attach a WebSocket listener to the same HTTP server. The `peerProxy.js` file and `peerProxy()` class contains the protocol upgrade from HTTP to WebSocket, keeps track of all WebSocket connections, passes or proxies requests between WebSocket connections, and implements ping/pong messages and interactions to keep WebSocket connections alive.
- **Frontend Making WebSocket Connection**: The `configureWebSocket()` function defines a raw socket and creates a new WebSocket object, sending an upgrade request and initiating a Websocket connection to the URL provided as an argument in the `play.js` file.
- **Data Sent over WebSocket Connection**: In the `play.js` file, the `broadcastEvent()` function is called within the `handleEmojiClick()` function to send an event message of other players' emoji reactions, the `endGame()` function to send an event message of other players' final scores upon ending each game, and the `socket.onopen` event to send an event message of other players upon starting a new game. The `broadcastEvent()` function sends an event message over the socket to all other players who have an alive WebSocket connection except for the sender of the message or the initiater of the event.
- **WebSocket Data Displaying in the Application Interface**: In the `play.js` file, whenever an event is broadcasted, the `socket.onmessage` event is triggered to call the `displayMsg()` function. Each WebSocket event message sent by other players is displayed at the top left corner of the game interface of the “Play" page/tab for the current player. In addition to that, a "game connected" or "game disconnected" message is also displayed when the WebSocket connection's `socket.onopen` event is triggered or when the WebSocket connection did not respond to the ping message last time.

# React Deliverable
*Note: The Login Deliverable is completed over the week of April 7th, 2024 and the week of April 15th, 2024. Please look for and refer to the git commits made for this deliverable during these two weeks.
- **Bundled Using Vite**: Vite is installed and configured as a development dependency and frontend tooling to bundle the code.
- **Multiple Functinal React Components**: An App React component is created as a top-level component that is render different view components. View components, including the login/home page (`login.jsx`), signup/register page (`signup.jsx`), play/game interface (`play.jsx`), and score and rank pages, are ported as React components to be rendered in the App component. The `login.jsx` also contains child components including `authenticated.jsx` and `unauthenticated.jsx`.
- **React Router**: React router is created in the top-level App component. The navigation routes and routed components are defined and injected using the `<BrowserRouter>`, `<NavLink>`, `<Routes>` and `<Route>` tags.
- **React Hooks**: The `useEffect()` hook is used in `authenticated.jsx` for the third-party API call to retrieve random lyrics of songs by keshi, in `play.jsx` to retrieve lyrics data that is used to display the lyric sentences as questions including the four possible answer choices, correct answer of the song name, and SoundCloud soundtrack link of the correct song from the backend server, in `score.jsx` to get and display the final score updated and stored in the browser's local storage for the current player, and in `rank.jsx` to retrieve the top ten highest scores from the service endpoint to display in the table on the rank page.
